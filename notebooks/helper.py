...
def build_wikipedia_url(city_name: str) -> str:
    """Builds Wikipedia URL for requesting."""
    return f"https://en.wikipedia.org/wiki/{city_name}".replace(" ", "_")

def scrape_city_soups_from_wikipedia(cities: list) -> list:
    """Collects responses for given urls."""
    responses = [requests.get(build_wikipedia_url(city_name)) for city_name, _ in cities]
    my_soups = [BeautifulSoup(response.content, 'html.parser') for response in responses]
    return my_soups

### -------- CITIES -------- ###

def create_city_df(cities: list) -> pd.DataFrame:
    """Creates initial city data frame from list of cities."""
    data = [{"city_name" : name, "city_country_code" : country_code} for name, country_code in cities]
...
def dms_string_to_decimal(dms_str: str) -> float:
    """Converts geographical information (lat/lon) to decimal."""
    deg = dms_str.split("°")[0]
    minutes = dms_str.split("°")[1].split("′")[0]
    if "\"" in dms_str:
        seconds = dms_str.split("′")[1].split("\"")
    else:
        seconds = None
    direction = dms_str[-1]
...
def create_geo_df_from_soups(cities: list, city_soups: list) -> pd.DataFrame:
    """Creates dataframe with geodata from given wikipedia responses."""
    city_data = []

    for city, soup in zip(cities, city_soups):
        city_name = city[0]
        country_code = city[1]
        latitude = soup.find('span', class_='latitude').get_text()
        longitude = soup.find('span', class_='longitude').get_text()
        data = {
            'city_latitude': dms_string_to_decimal(latitude),
            'city_longitude': dms_string_to_decimal(longitude)
        }
...
def extract_population_from_string(string: str) -> int:
    """Convert string to numeric population."""
    string = string.strip()
    population = re.match(r"[\d,]+", string).group(0)
    population = int(population.replace(",", ""))
    return population

def create_population_df_from_soups(cities: list, city_soups: list) -> pd.DataFrame:
    """Creates dataframe with population data from given wikipedia responses."""
    city_data = []
    ...
    for city, soup in zip(cities, city_soups):
        ...
        city_population = infobox.find(string="Population").find_next("td").get_text()
        city_population_clean = extract_population_from_string(city_population)
...
def get_airports(
    ...
    """Creates data frame with all airport data for given city geo data."""
    # API parameters
    headers = {
        "X-RapidAPI-Key": api_config.get("flights").get("key"),
        "X-RapidAPI-Host": api_config.get("flights").get("host")
    }
    ...
    for city_id, lat, lon in zip(city_ids, latitudes, longitudes):
